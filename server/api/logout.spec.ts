...
describe('POST /api/logout', () => {
  it('returns ok without session', async () => {
    const res = await $fetch<{ ok: boolean }>('/api/logout', {
      method: 'POST'
    })
  ...
  it('login then me then logout then me is 401', async () => {
    ...
    const me1 = await fetch(url('/api/me'), { headers: { Cookie: cookieHeader } })
    ...
    const me2 = await fetch(url('/api/me'), { headers: { Cookie: cookieHeader } })
